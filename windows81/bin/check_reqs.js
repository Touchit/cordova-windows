...
function CheckSystem() {
    ...
    var fail_msg = 'The command `msbuild` failed. Make sure you have the latest Windows 8.1 SDKs installed, and the `msbuild.exe` command (inside C:\\Windows\\Microsoft.NET\\Framework\\v4.0.30319) is added to your path.'
    var output = check_command(cmd, fail_msg);
    var msversion = output.match(/\.NET\sFramework\,\sversion\s4\.0/);
    if (!msversion) {
        Log('Please install the .NET Framwork v4.0.30319 (in the latest windows 8.1 SDK\'s).', true);
